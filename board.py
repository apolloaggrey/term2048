...
class Board(object):
    ...
    UP, DOWN, LEFT, RIGHT, PAUSE, UNDO = 1, 2, 3, 4, 5, 6
    SUP, SDOWM, SLEFT, SRIGHT = 11, 12, 13, 14
    HISTORY = []
    GOAL = 2048
    SIZE = 5

    def __init__(self, goal=GOAL, size=SIZE, history=HISTORY, **kws):
    ...
    def board_to_line(self):
        line = []
        for y in self.__size_range:
            for x in self.__size_range:
                line.append(self.getCell(x, y))
        print(line)
        return line

    def export_board_state(self):
        with open("board_state.csv", mode="w") as file:
            for x in self.board_to_line():
                file.write(str(x)+",")

    def load_to_history(self):
        """save last move to history"""
        if len(self.history) < self.__size*2:
            self.history.append(self.board_to_line())
        else:
            self.history.pop(0)
            self.history.append(self.board_to_line())

    def load_from_history(self):
        """load previous board state from history"""
        if len(self.history) > 0:
            self.history.pop(-1)
            line = self.history.pop(-1)
            for y in self.__size_range:
                for x in self.__size_range:
                    self.setCell(x, y, int(line[x + (y * self.__size)]))
        else:
            pass  # end of history
